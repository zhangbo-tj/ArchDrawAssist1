...
void GLModel::CalAverage(){
	...
	if(verNum <1)
		return;
	sumx = mVertices[0].GetX();	sumy = mVertices[0].GetY();	sumz = mVertices[0].GetZ();
	mMaxPt[0] = mVertices[0].GetX();	mMaxPt[1] = mVertices[0].GetY();	mMaxPt[2] = mVertices[0].GetZ();
	mMinPt[0] = mVertices[0].GetX();	mMinPt[1] = mVertices[0].GetY();	mMinPt[2] = mVertices[0].GetZ();

	for(int i= 1;i< verNum;i++){
		sumx += mVertices[i].GetX();
		sumy += mVertices[i].GetY();
		sumz += mVertices[i].GetZ();
		mMaxPt[0] = mVertices[i].GetX() > mMaxPt[0] ? mVertices[i].GetX():mMaxPt[0];
		mMaxPt[1] = mVertices[i].GetY() > mMaxPt[1] ? mVertices[i].GetY():mMaxPt[1];
		mMaxPt[2] = mVertices[i].GetZ() > mMaxPt[2] ? mVertices[i].GetZ():mMaxPt[2];

		mMinPt[0] = mVertices[i].GetX() < mMinPt[0] ? mVertices[i].GetX():mMinPt[0];
		mMinPt[1] = mVertices[i].GetY() < mMinPt[1] ? mVertices[i].GetY():mMinPt[1];
		mMinPt[2] = mVertices[i].GetZ() < mMinPt[2] ? mVertices[i].GetZ():mMinPt[2];
	}
...
void GLModel::Unitlize(){
	...
	for(int i = 0;i < mVertices.size();i++){
		float vx = mVertices[i].GetX();
		float vy = mVertices[i].GetY();
		float vz = mVertices[i].GetZ();
...
void GLModel::CalFacetNormals(){
	for(int i = 0;i<mTriangles.size();i++){
		int* vindices =mTriangles[i].GetVindices();
...
void GLModel::GenIncidentTable(){
	...
	for(int i =0;f_iter != mTriangles.end()&& i <  numTri;i++,f_iter++){
		int pa,pb,pc;
		int* vindices = f_iter->GetVindices();
		pa = vindices[0];
		pb = vindices[1];
		pc = vindices[2];
...
GLModel* GLMReadOBJ(const char* filename){
	...
	while(fscanf(file,"%s",buf) != EOF){
		...
		case 'g':		//去掉设置group的部分
			fgets(buf,sizeof(buf),file);
			sscanf(buf,"%s",buf);
			if(model->mGroups.size()==1&&strcmp(model->mGroups[0].GetName(),"")==0){
		...
		case 'f':
			...
			if(strstr(buf, "//")){
				...
				while(fscanf(file, "%d//%d",&v,&n)>0){
					...
					t_vindices = model->mTriangles[curIndex].GetVindices();
					t_nindices = model->mTriangles[curIndex].GetNindices();
				...
				while(fscanf(file,"%d/%d/%d",&v,&t,&n)>0){
					...
					t_vindices = model->mTriangles[curIndex].GetVindices();
					t_nindices = model->mTriangles[curIndex].GetNindices();
					t_tindices = model->mTriangles[curIndex].GetTindices();
...
int GLMFindMaterial(GLModel* model, char* name){
	...
	for (i = 0; i < model->mMaterials.size();i++){
		if(!strcmp(model->mMaterials[i].GetName(),name))
			return i;
	}
...
void GLMDraw(GLModel* model, GLuint mode){
	...
	for(unsigned int i = 0;i < model->mGroups.size();i++){
		...
		Material material = model->mMaterials.at(group.GetMaterialno());
		if(mode & GLM_MATERIAL){
			...
			glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, material.GetShiness());
		}

		if (mode & GLM_COLOR) {
			glColor3fv(model->mMaterials[0].GetDiffuse());
		}

		/* add code here to bind texture for each model */
		if (mode & GLM_TEXTURE) {
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, model->mTextureArray[group.GetMaterialno()]);
		}

		glBegin(GL_TRIANGLES);
		for(unsigned int j= 0;j< group.GetTriangles().size();j++){
			Triangle triangle = model->mTriangles.at(group.GetTriangles().at(j));
			vindices = triangle.GetVindices();
			nindices = triangle.GetNindices();
			tindices = triangle.GetTindices();

			if(mode & GLM_FLAT)
				glNormal3fv(model->mFacetNormals[triangle.GetFindex()].ToFloat());
			if(mode * GLM_SMOOTH)
			{
				
				glNormal3fv(model->mNormals.at(nindices[0]).ToFloat());
			}
			if(mode & GLM_TEXTURE){
				glTexCoord2fv(model->mTexcoods.at(tindices[0]).ToFloat());
			}

			glVertex3fv(model->mVertices.at(vindices[0]).ToFloat());

			if(mode * GLM_SMOOTH)
				glNormal3fv(model->mNormals[nindices[1]].ToFloat());
			if(mode & GLM_TEXTURE)
				glTexCoord2fv(model->mTexcoods[tindices[1]].ToFloat());
			glVertex3fv(model->mVertices[vindices[1]].ToFloat());

			if(mode * GLM_SMOOTH)
				glNormal3fv(model->mNormals[nindices[2]].ToFloat());
			if(mode & GLM_TEXTURE)
				glTexCoord2fv(model->mTexcoods[tindices[2]].ToFloat());
			glVertex3fv(model->mVertices[vindices[2]].ToFloat());
...
void GLMWriteOBJ(GLModel* model, char* filename,GLuint mode){
	...
	if(mode & GLM_SMOOTH){
		...
		for(unsigned int i= 0;i < model->mNormals.size();i++){
			...
			fprintf(file,"vn %f %f %f",normal.GetX(),normal.GetY(),normal.GetZ());
		}
	...
	if(mode & GLM_TEXTURE){
		...
		for(unsigned int i = 0;i< model->mTexcoods.size();i++){
			...
			fprintf(file,"vt %f %f",texture.GetX(),texture.GetY());
		}
	}
	...
	for(unsigned i = 0;i< model->mGroups.size();i++){
		...
		fprintf(file,"g %s \n",group.GetName());
		if(mode & GLM_MATERIAL){
			fprintf(file,"usemtl %s\n",model->mMaterials.at(group.GetMaterialno()).GetName());
			for(unsigned int j = 0;j < group.GetTriangles().size();j++){
				if(mode & GLM_SMOOTH && mode && GLM_TEXTURE){
					triangle = model->mTriangles.at(group.GetTriangles().at(j));
					vindices = triangle.GetVindices();
					nindices = triangle.GetNindices();
					tindices = triangle.GetTindices();
